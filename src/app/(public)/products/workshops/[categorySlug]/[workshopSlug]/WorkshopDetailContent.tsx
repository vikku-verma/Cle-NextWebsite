...
export default function WorkshopDetailContent({ params }: Props) {
    const { workshops } = useWorkshops();
    const { categorySlug, workshopSlug } = params;

    // Find the workshop in the shared state (cached)
    const workshop = useMemo(() => {
        return workshops.find(w => w.name === workshopSlug || String(w.id) === workshopSlug);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
                <div className="text-center">
                    <h2 className="text-2xl font-bold text-slate-800 mb-4">Workshop Not Found</h2>
                    <Link href="/products/workshops" className="text-amber-600 hover:underline">
                        Back to Workshops
                    </Link>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div>
                            ...
                            {workshop.registrationEndDateRaw && (
                                <div>
                                    <CountdownTimer
                                        endDate={workshop.registrationEndDateRaw}
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="relative flex justify-center items-center h-full">
                            {workshop.heroImage ? (
                                <div className="relative h-[80%] w-auto max-w-full rounded-2xl overflow-hidden shadow-2xl aspect-[16/9] min-w-[300px]">
                                    <NextImage
                            ...
                            <div className="bg-[#0f172a] rounded-2xl p-6 shadow-lg">
                                ...
                                <div className="space-y-4 text-white">
                                    {workshop.fees && workshop.fees.length > 0 ? (
                                        workshop.fees.map((fee, index: number) => (
